...
function showFileOpenDialog () {
	...
	elem.innerHTML = '<form method="post" enctype="multipart/form-data" style="position: absolute; left: 50%; margin-left: -250px; margin-top: 50px; width: 500px; height: 150px; padding: 10px; background-color: #A65E00;">' +
	...
	var processor = setInterval(function() {
		if ( i <= 50){
			document.getElementById("fileopen").setAttribute('style', 'opacity: ' + (i/50.0) + ';');
		}
		else {
...
function closeFileOpenDialog () {
	...
	var processor = setInterval(function() {
		if ( i <= 50){
			document.getElementById("fileopen").setAttribute('style', 'opacity: ' + (1.0 - (i/50.0)) + ';');
		}
		else {
			var elem = document.getElementById("fileopen");
			elem.innerHTML = "";
